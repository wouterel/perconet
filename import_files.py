    ...
    if b is None or bl is None or bb is None:
        ...
        while d and dl and db:
            if b.meta.t == bl.meta.t:
                ...
                print ('bug! cannot synch files', bl.meta.t,bb.meta.t,b.meta.t)
                return None, None, None
                 
def synchro(d, dr, db):
#synchronizes the input files
#d = dump file of the centers
#dr = dump file of the reversible ends
#di = dump file of the irreversible ends
#db = custom dump file containing the number of reversible bonds
    br = dr.next_block()
    b = d.next_block()
    bb = db.next_block()
    ...
    if b is None or br is None or bb is None:
        ...
        if b.meta.t == br.meta.t == bb.meta.t:
            synched = True
            #print( "synched!, ", b.meta.t )
        else:
            synched = False

        while not synched:
            max_t = np.max([b.meta.t, br.meta.t, bb.meta.t])
            if b.meta.t < max_t:
                print('center files behind, t=', b.meta.t, 'tb=', max_t)
                b = d.next_block()
            elif br.meta.t < max_t:
                print('reversible behind, t=', br.meta.t, 'tb=', max_t)
                br = dr.next_block()
            elif bb.meta.t < max_t:
                print('bond file behind, t=', bb.meta.t, 'tb=', max_t)
                bb = db.next_block()

            if b.meta.t == br.meta.t == bb.meta.t:
                synched = True
            elif b is None or br is None or bb is None:
                print('bug! cannot synch files')
                return None, None, None
        #print( "synched!, returning b, br, bb")    
        return b, br, bb
